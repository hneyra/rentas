 ...
 *
 * Tres `props` que hay que pasar **igual** en todos los sitios que dibujan una pantalla —la
 ...
 * se escriben una vez, y lo que la guarda monta es lo que se sirve.
 *
 * **No es una copia del interprete**: no dibuja nada. Lo vigila
 * `verificaciones/el-interprete-es-de-la-libreria.test.ts`.
 */
export type PantallaDeRentasProps = Omit<PantallaProps, 'traducir' | 'textos' | 'tonoDeLaInsignia'>;
...
export function PantallaDeRentas(props: PantallaDeRentasProps) {
  ...
  return (
    <Pantalla {...props} traducir={(texto) => t(texto)} textos={textos} tonoDeLaInsignia={tonoDe} />
  );
}
