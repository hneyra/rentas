> **NOTA DEL CORTE (P5A).** De `sgtm/infra/` sólo viaja a `rentas` este directorio: los
> **archivos de ejemplo** y este README. No viajan los guiones (`sembrar-demostracion.sh`,
> `cargar-cajas.sh`, `publicar-parametros.sh`…) ni el descriptor de Pulumi — eso es
> `infrastructure`, y la siembra se replantea cuando cada sistema tenga su propio despliegue.
> Los CSV están aquí porque **el backend los lee en sus pruebas**: `ArchivosDeEjemploTest`,
> `ArchivoDeContribuyentesDeEjemploTest` y `CarteraCuadraConLaConsultaJdbcTest` los pasan por el
> analizador de verdad, así que dejarlos atrás no habría quitado un guion: habría quitado
> catorce pruebas. **Lo que este README describe de los guiones, aquí todavía no se puede
> ejecutar.**
