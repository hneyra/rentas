...
#        --archivo ejemplos/deuda.csv [--namespace kamayuk-stg] [--observacion "..."]
#
...
SUFIJO=$(date +%s)
RECURSO="kamayuk-${AMBIENTE}-carga-demo-deuda-${SUFIJO}"
            ...
            - name: KAMAYUK_DB_CLAVE
              valueFrom:
                secretKeyRef:
                  ...
                  key: clave-app
            - name: KAMAYUK_CARGADEUDADEMO_MUNICIPALIDADID
              value: "$MUNICIPALIDAD_ID"
            - name: KAMAYUK_CARGADEUDADEMO_ARCHIVO
              value: /datos/deuda.csv
            - name: KAMAYUK_CARGADEUDADEMO_USUARIODELPROCESO
              value: carga-demostracion
            - name: KAMAYUK_CARGADEUDADEMO_OBSERVACION
              value: "$OBSERVACION"
          volumeMounts:
            - name: datos
              mountPath: /datos
