...
| Su frontend (`rentas-web`) | **Existe, y desde #98 la interfaz que se sirve es la de RentasV8.** Vite 7 + React 19.3 + TypeScript 5.9 en `frontend/`, con el codigo en `frontend/src/` —y no en un monorepo, porque `verificar-fila-del-registro.mjs` declara `/^frontend\/src\//` como codigo de produccion—. `yarn verificar`: **423 pruebas en 25 archivos**, 0 fallos; `yarn build` produce **690 kB de JS y 9,3 kB de CSS**. La V6 salio entera: **26 018 lineas borradas** —`src/estilos/` (3 446), `src/ds/`, `src/marco/`, `src/secciones/`, el prototipo, el proxy de datos, su artboard y `e2e/`—, y la caida de 1 296 pruebas a 423 es su suite yendose con ella.<br><br>**Y NO INSTALA SIN SU CLON HERMANO.** `frontend/package.json` declara **cinco** `link:../../kamayuk-lib/paquetes/*` —`@kamayuk/{api,formato,sesion,shell,ui}`— que yarn resuelve **contra el disco**. Si falta: `git clone https://github.com/hneyra/kamayuk-lib ../../kamayuk-lib`. Ojo: `yarn install --frozen-lockfile` con el hermano ausente sale con **codigo 0** y deja enlaces colgantes; quien lo dice, nombrando el `git clone`, es `verificaciones/enlace-con-kamayuk-lib.test.ts`. **Y hay dos cosas mas que el enlace exige y que se descubrieron por las malas** (#88): `preserveSymlinks: true` en `tsconfig.base.json` —sin el, en CI `tsc` resuelve los `import` de la libreria contra el arbol del hermano, que alli NO tiene dependencias— y `resolve.dedupe` en `vite.config.ts` con la lista **derivada de las `peerDependencies`** (`resolucion.ts`), sin la cual salen dos copias de React. El mismo defecto con sintomas opuestos segun donde se mire.<br><br>**Que hay hoy**: `src/aplicacion.tsx` monta `<Armazon>` de `@kamayuk/shell` —barra global, carril de diez modulos, paleta `Ctrl/Cmd+K`, menu de sesion y el aviso de salir sin guardar— con `src/catalogo.ts`, que traduce el arbol de este sistema a la forma generica **y deduce el icono del trazo** en vez de escribirlo. El cuerpo de cada pantalla lo dibuja `src/pantallas/Pantalla.tsx`, el **interprete**, desde una de las 40 definiciones tipadas. Tailwind v4 procesa de verdad desde #90.<br><br>**Lo que ata la interfaz al artboard son cinco guardas**: la paleta token a token, las 40 pantallas **campo por campo** contra `diseno/RentasV8.dc.html`, que **toda clase de Tailwind produzca una regla** —y no solo este escrita— (#91), que el interprete **no nombre ningun sistema** —esta destinado a `@kamayuk/ui`—, y que **la V6 no vuelva** al arbol (#90): un borrado se deshace sin querer y no rompe nada.<br><br>**⚠️ Dos cosas que NO estan, y hay que saberlo**: (1) **las pantallas no piden datos**. Dibujan las cifras de su propia definicion, que son las del artboard, **y por tanto viajan en el bundle** — la garantia de #44 esta ABIERTA, dicha en el `Dockerfile` con su numero y vigilada para que no se olvide (**#97**). (2) **El arnes de extremo a extremo salio con las pantallas contra las que corria**, y su trabajo de CI con el; vuelve con el arnes nuevo. Lo que SI sigue: entrar con codigo de autorizacion y PKCE contra Keycloak (`src/arranque.ts`), y que `yarn build` no publique codigo fuente en la imagen |
...
frontend/               Vite 7, React 19, TypeScript 5.9, con yarn. `rentas-web`
  ...
  src/pantallas/                  las 40 pantallas como dato, el arbol, y el INTERPRETE que las dibuja
  ...
  src/datos/                      el contrato de ESTA API: `lecturas.ts`, las 12 de `servidas.ts`
...
declara `@kamayuk/{api,formato,sesion,shell,ui}` como `link:../../kamayuk-lib/paquetes/*`, que yarn resuelve
