...
| `backend/` — **17 módulos** | **Existe, y la resta terminó (P5E).** `./gradlew build` en verde: **3 459 pruebas**, 0 fallos —medido el 2026-09-20 contando los `TEST-*.xml` de los diecisiete modulos; es una lectura de ESE dia y no un contrato, y la renueva quien vuelva a correr el comando—. Es el monolito modular menos la interfaz, menos los valores normativos (P5B), menos el catastro (P5C) y menos la ventanilla (P5D). De los 17 módulos, **quince son de este sistema** y dos —`kamayuk-rentas-catastro` y `kamayuk-rentas-parametros`— son **adaptadores cliente**: puertos y transporte, sin dominio y sin una sola consulta a una tabla ajena |
...
| `backend/kamayuk-rentas-aplicacion` | **Existe.** `verificarArquitectura` corre **268 pruebas** (2026-09-20): las barreras de la librería común más las propias de este sistema (contrato, formas, respuestas, límites de Modulith) |
...
ellas. Lo que hoy vigilan es real: 3 459 pruebas sobre los contextos que quedan.
...
frontend/               Vite 7, React 19, TypeScript 5.9, con yarn. `rentas-web`
  ...
  src/datos/                      el contrato de ESTA API: `lecturas.ts`, las 36 de `servidas.ts`
