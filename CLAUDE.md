...
| Su frontend (`rentas-web`) | **Existe el andamiaje, el vocabulario visual, sus datos de prototipo, el MARCO V6 y el módulo «Rentas · Registro» COMPLETO — sus cuatro secciones** (F-1 #3, F-2 #4, F-4 #6, F-3 #5, F-5 #7, F-6 #8). Vite 7 + React 19 + TypeScript 5.9 en `frontend/`, con el código en `frontend/src/` —y no en un monorepo, porque `verificar-fila-del-registro.mjs` declara `/^frontend\/src\//` como código de producción—. `yarn verificar` encadena lint, tipos y pruebas: **1 110 pruebas** en 43 archivos, 0 fallos; `yarn build` produce 286 kB de JS y 44 kB de CSS. Sus reglas son **nueve prohibiciones de ESLint, cada una con su muestra que la viola**, y una prohibición sin muestra sale roja sola. **Sus tokens son los del artboard V6** —no los de la etapa anterior— y una prueba compara sus hex contra `frontend/diseno/RentasV6.dc.html`, que viaja vendorizado. **Y tiene datos** (F-4): un **proxy que sustituye `globalThis.fetch`** y contesta **dieciocho** de las 162 operaciones del contrato —eran 181 hasta la etapa 4 de ADR-0039— con los datos del artboard `RentasV6`, detrás de `VITE_KAMAYUK_PROXY_DE_DATOS` y por `import()` dinámico — con la bandera apagada no viaja al bundle ni una cifra del artboard, medido. **Y desde F-3 tiene marco** (`src/marco/`): la barra global, el árbol de **diez módulos y 40 destinos**, las pestañas con su estado sin guardar, el enrutado por hash con `replaceState` y la paleta de `Ctrl/Cmd+K`. Del artboard se porta **sólo la variante A** y el conmutador A/B/C no existe en el código, que lo vigila un escáner de fuentes. **Y tiene sus CUATRO secciones** (`src/secciones/`): el **panel** del módulo (F-5), **Contribuyentes** con su expediente y su alta guiada (F-5), **Determinación** —los seis tipos de cálculo, con la escala progresiva completa del predial y las cinco etapas de la emisión masiva— y **Valores** —la portada del conjunto sellado del ejercicio, con sus tres pestañas— (F-6). Las cuatro leen por HTTP a través del proxy y **ninguna importa la captura del artboard**: con la bandera apagada, el bundle sigue sin llevar una sola cifra suya, medido con `grep` sobre el `dist`. **El módulo recorre entero**: una prueba abre las cuatro por su slug y exige de cada una algo que sólo aparece con los datos puestos. **Y desde I-1 (#24) e I-3 (#31) está CONECTADO**: entra con código de autorización y PKCE contra Keycloak, y **seis** operaciones salen a la red de verdad —las dos de sesión, los módulos, los accesos, la matriz de permisos y la **primera escritura**, `PUT /seguridad/sesion/ejercicio`—; el resto lo sigue contestando el proxy. El árbol del marco **ya no es la constante del artboard**: lo compone `marco/composicion.ts` de `GET /seguridad/modulos` —de ahí salen qué módulos hay, su orden y su rótulo—, el catálogo aporta el icono y los cuatro destinos, y lo que la cuenta no puede abrir no se ofrece en ninguna de las tres listas ni por el hash. **Y desde I-2 (#28) tiene ARNÉS de extremo a extremo** (`frontend/e2e/`, Playwright sobre Chromium, `yarn e2e`): **21 caminos** contra la instalación levantada —no contra el proxy de datos—, que entran por el formulario de Keycloak de verdad, con PKCE y canje; cubren los cuatro peldaños alcanzables de la escalera de identidad y una sección cada uno, y exigen de cada pantalla algo que sólo tiene el volcado de la marcha blanca. No corre en `yarn verificar` ni en CI: lo primero a propósito (AC8) y lo segundo declarado en `frontend.yml` con las cuatro cosas que le faltan escritas dentro |
