...
BEGIN
    ...
    -- `catastro` (V4), y existe SEPARADO de `kamayuk_app` a proposito. Que la proyeccion sea de
...
--  pudo hacer: `REVOKE CONNECT` es sobre la BASE, y `kamayuk_owner` no la posee). `rol_ingestor_catastro`
...
BEGIN
    ...
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO kamayuk_owner, kamayuk_app, kamayuk_readonly, rol_ingestor_catastro', base);
END
$connect$;
